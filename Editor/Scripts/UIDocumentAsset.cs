...
internal class UIDocumentAsset
{
    ...
    public UIDocumentAsset(string uiDocumentPath)
    {
        ...
        BindingsOutputFile = BuildOutputFile(uiDocumentPath);
    }
    ...
    private string BuildOutputFile(string uiDocumentPath)
    {
        string autogeneratedFolder = System.IO.Path.Combine("Assets", "Rosalina", "AutoGenerated");

        var name = System.IO.Path.GetFileNameWithoutExtension(uiDocumentPath);
        var path = System.IO.Path.GetDirectoryName(uiDocumentPath);
        ...
        var newPath = System.IO.Path.Combine(path, $"{name}.g.cs");
